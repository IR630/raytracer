 ...
 *               Windows main project handle module.
...
namespace vart
{
  ...
  private:
    ...
    BOOL IsFullScreen;       // flag fullscreen
    RECT FullScreenSaveRect; // full screen save
    INT MouseWheel;          // mouse wheel variable
  public:
    BOOL IsActive;           // is active window
     ...
     *       HINSTANCE hInst;
     */
    win( HINSTANCE hInst = GetModuleHandle(nullptr) ) : W(0), H(0), IsFullScreen(FALSE), 
                                                        IsActive(TRUE), FullScreenSaveRect({}), 
                                                        hWnd(), MouseWheel(0), hInstance(hInst)
    {
